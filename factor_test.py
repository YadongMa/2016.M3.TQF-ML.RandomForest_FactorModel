...
def optimizer(w0, factors, R,indusnum, div_Indus, div_TV, wbench, maxweight, dateid):
    invalidx = np.vstack([HighPrice[dateid,:]==LowPrice[dateid,:] , np.isnan(HighPrice[dateid,:]) , SpecialTreat[dateid-1,:]==1 , ListDays[dateid-1,:]<360]).any(axis = 0)        #invalid的股票，向量
    wend = wbench
    alphascores = factors[0,:]
    expose_TV = factors[1,:]
    indus_weight = np.zeros([28,1])
    
    for i in range(28):
        indus_weight[i] = np.sum(wbench[indusnum==i])

    num1 = alphascores.shape[1]
    id1 = np.where(w0>0)
    id2 = np.where(alphascores>-1000000)
    id3 = np.where(wbench>0)
    idx = np.union1d(np.union1d(id1,id2),id3)
    alphascores[np.isnan(alphascores)] = np.min(alphascores)-1
    expose_TV[np.isnan(expose_TV)] = 0
              
    R = R[idx,:]        #行业虚拟变量中剔除无效的行
    num = len(idx)        #num为有效股票的数量
    wbench = wbench/np.sum(wbench)
    wbench = wbench[idx]
    w0 = w0[idx]
    expose_TV = expose_TV[idx]
    invalidx = invalidx[idx]
    alphascores = alphascores[idx]
    indusnum = indusnum[idx]
    
    # bound
    UB = np.ones([num,1]) * maxweight - wbench       #t为单个股票的最大权重
    LB = np.zeros([num,1]) - wbench         #单个股票的最小权重为0
    indus_count = np.zeros([28,1])
    max_indus_weight = np.zeros([28,1])

    
    # 调整个股上限
    for i in range(28):
        indus_count[i] = np.sum(indusnum[invalidx==0]==i)
        max_indus_weight[i] = indus_count[i] * maxweight
        if max_indus_weight[i] < indus_weight[i]:
            UB[indusnum==i] = indus_weight[i]/indus_count[i] - wbench[indusnum==i]
     
    UB[invalidx>0] = w0[invalidx>0] - wbench[invalidx>0]
    LB[invalidx>0] = w0[invalidx>0] - wbench[invalidx>0]

    
    R = R.T         #在做行业偏离度约束时的不等式系数
    I1 = expose_TV      #在做市值约束时的不等式系数
    ...
    # ineq constraint（不等式约束）
    A = np.vstack([R,-R,I1,-I1])
    b = np.concatenate([div_Indus*np.ones([R.shape[0],1]),div_Indus*np.ones([R.shape[0],1]),[div_TV,div_TV]])
